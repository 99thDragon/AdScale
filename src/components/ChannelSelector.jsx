...
import { labelCaps } from '../styles/ui'
...
function ChannelSelector({ selectedIds, onChange }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className={labelCaps}>Launch channels</p>
      ...
      {Object.entries(byPlatform).map(([platform, channels]) => (
        <div key={platform}>
          <p className="mb-2 text-sm font-medium text-ink">{platform}</p>
      ...
      {selectedIds.length === 0 && (
        <p className="text-sm text-warm">Select at least one channel to launch.</p>
      )}
    </div>
  )
}
